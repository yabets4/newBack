...
export default class OnboardingController {
  ...
  static async listTemplates(req, res, next) {
    try {
      ...
      const rows = await service.listTemplates(companyId);
      return ok(res, rows);
    } catch (e) { next(e); }
  }
  ...
  static async getTemplate(req, res, next) {
    try {
      const companyId = req.auth.companyID;
      const rec = await service.getTemplate(companyId, req.params.id);
      if (!rec) return notFound(res, 'Template not found');
      return ok(res, rec);
    } catch (e) { next(e); }
  }
  ...
  static async deleteTemplate(req, res, next) {
    try {
      const companyId = req.auth.companyID;
      await service.deleteTemplate(companyId, req.params.id);
  ...
  static async listProcesses(req, res, next) {
    try {
      const companyId = req.auth.companyID;
      const opts = { employee_id: req.query.employee_id, status: req.query.status };
      const rows = await service.listProcesses(companyId, opts);
      return ok(res, rows);
    } catch (e) { next(e); }
  }
  ...
  static async getProcess(req, res, next) {
    try {
      const companyId = req.auth.companyID;
      const rec = await service.getProcess(companyId, req.params.id);
      if (!rec) return notFound(res, 'Process not found');
      return ok(res, rec);
    } catch (e) { next(e); }
  }
  ...
  static async deleteProcess(req, res, next) {
    try {
      const companyId = req.auth.companyID;
      await service.deleteProcess(companyId, req.params.id);
      return ok(res, { message: 'Deleted' });
    } catch (e) { next(e); }
  }
  ...
  static async updateTaskStatus(req, res, next) {
    try {
      ...
      const updated = await service.updateTaskStatus(companyId, id, taskId, status);
      if (!updated) return notFound(res, 'Process or task not found');
      return ok(res, updated);
    } catch (e) { next(e); }
  }
}
